...
class CustomPostController(http.Controller):
    ...
    def handle_post_request(self, **post_data):
        try:
            uid = request.session.authenticate('Jan_23', 'admin', '.')
            account_move_update = request.env['account.move'].search([('id','=',post_data.get('id'))])
    ...
    def action_register_payment_automatically(self,payment):
        ...
        payment_method = payment.env['account.payment.method'].search([('code', '=', 'manual')],
