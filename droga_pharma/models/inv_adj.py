...
class droga_inv_adj(models.Model):
    ...
    x_has_access = fields.Boolean('is_adj_available', default=False, compute='_compute_has_access',
    ...
    def _search_has_access(self, operator,value):
        if operator == '=' and len(self.env['res.groups'].search([('name', 'in', ('Pharmacy finance manager','Finance inventory workflow approver'))]))>0:
            x_has_access = self.env['droga.stock.adjustment.request'].sudo().search([('order_from','=','PH'),('state','in',('waiting','processed','done','finmgp'))])
