...
class ReportWizard(models.TransientModel):
    ...
    def generate_xls_report(self, workbook, excel_data):
        ...
        row = 2
        col = 0
        sheet.write(row, col + 0, 'Client', bold)
        sheet.write(row, col + 1, 'Customer', bold)
        sheet.write(row, col + 2, 'Indication', bold)
        sheet.write(row, col + 3, 'Drug Therapy Problem', bold)
        sheet.write(row, col + 4, 'Drug Therapy Cause', bold)
        sheet.write(row, col + 5, 'Intervention', bold)
        sheet.write(row, col + 6, 'Intervention Implemented', bold)
        sheet.write(row, col + 6, 'Assessment and Care Plan', bold)
        sheet.write(row, col + 6, 'Intervention/ Recommendation', bold)

        # Iterate over excel_data and write the values to the sheet
        for index, ed in enumerate(excel_data):
            row = index + 3
            sheet.write(row, col + 0, re.search(r"'([^']*)'", str(ed.get('client', ''))).group(1))
            sheet.write(row, col + 1, re.search(r"'([^']*)'", str(ed.get('customer', ''))).group(1))
            ...
            sheet.write(row, col + 4, re.search(r"'([^']*)'", str(ed.get('drug_therapy_cause', ''))).group(1))
            sheet.write(row, col + 5, re.sub('<[^<]+?>', '', str(ed.get('intervention', ''))))
            sheet.write(row, col + 6, re.sub('<[^<]+?>', '', str(ed.get('intervention_implemented', ''))))
            sheet.write(row, col + 6, re.sub('<[^<]+?>', '', str(ed.get('asses_care_plan', ''))))
            sheet.write(row, col + 6, re.sub('<[^<]+?>', '', str(ed.get('recs_inter', ''))))
