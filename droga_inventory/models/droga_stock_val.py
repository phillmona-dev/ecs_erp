            ...
            if ret.origin.startswith('SO'):
                ...
                for mv in acc_move:
                    ...
                    for mvld in mvl:
                        ...
                        val_layers = self.env['droga.stock.valuation.layer'].search(
                            [('stock_move_id', 'in', stock_move_ids.ids)])
                        if len(val_layers) == 0:
                            mvld.sales_cost = 0
                        else:
    ...
    def _validate_accounting_entries_custom(self):
        ...
        for svl in self:
            ...
            am_vals += move.with_company(svl.company_id)._account_entry_move_custom(svl.quantity, svl.description,
                                                                                    svl.id, svl.value)
        for val in am_vals:
            self.con_acc = val['line_ids'][0][2]['account_id'] if val['line_ids'][0][2][
                                                                      'account_id'] != self.inv_acc.id else \
            val['line_ids'][1][2]['account_id']
        if am_vals:
    ...
    def get_purch_grn_total(self):
        for rec in self:
            ...
            if rec.target_model == 'pos':
                ...
                rec.lc_rate = ((
                                           rec.amount_total + rec.purchase_total) / rec.purchase_total) if rec.purchase_total != 0 else 1
        ...
        if 'price_unit' in vals or 'product_qty' in vals:
            for rec in self:
                ...
                for move in rec.mapped('move_ids'):
                    if move.state == 'done' and self.env.user.id != 114: #mersert hiruy
                        raise UserError(
    ...
    def _account_entry_move_custom(self, qty, description, svl_id, cost):
        ...
        if self._is_in():
            if self._is_returned(valued_type='in'):
                am_vals.append(
                    self.with_company(company_to).with_context(is_returned=True)._prepare_account_move_vals_custom(
                        acc_dest, acc_valuation, journal_id, qty, description, svl_id, cost))
            else:
                am_vals.append(
                    self.with_company(company_to)._prepare_account_move_vals_custom(acc_src, acc_valuation, journal_id,
        ...
        if self._is_out():
            ...
            if self._is_returned(valued_type='out'):
                am_vals.append(
                    self.with_company(company_from).with_context(is_returned=True)._prepare_account_move_vals_custom(
                        acc_valuation, acc_src, journal_id, qty, description, svl_id, cost))
            else:
                am_vals.append(
                    self.with_company(company_from)._prepare_account_move_vals_custom(acc_valuation, acc_dest,
        ...
        if self.company_id.anglo_saxon_accounting:
            ...
            if self._is_dropshipped():
                if cost > 0:
                    am_vals.append(
                        self.with_company(self.company_id)._prepare_account_move_vals_custom(acc_src, acc_valuation,
                                                                                             journal_id, qty,
                                                                                             description, svl_id, cost))
                else:
                    cost = -1 * cost
                    am_vals.append(
                        self.with_company(self.company_id)._prepare_account_move_vals_custom(acc_valuation, acc_dest,
                                                                                             journal_id, qty,
                                                                                             description, svl_id, cost))
            elif self._is_dropshipped_returned():
                if cost > 0 and self.location_dest_id._should_be_valued():
                    am_vals.append(self.with_company(self.company_id).with_context(
                        is_returned=True)._prepare_account_move_vals_custom(acc_valuation, acc_src, journal_id, qty,
                                                                            description, svl_id, cost))
                elif cost > 0:
                    am_vals.append(self.with_company(self.company_id).with_context(
                        is_returned=True)._prepare_account_move_vals_custom(acc_dest, acc_valuation, journal_id, qty,
                                                                            description, svl_id, cost))
                else:
                    cost = -1 * cost
                    am_vals.append(self.with_company(self.company_id).with_context(
