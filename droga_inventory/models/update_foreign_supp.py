...
class update_foreign(models.Model):
    ...
    def update_suppliers(self):
        supp=self.env['purchase.order'].search([('request_type','=','Foregin'),('company_id','=',1)]).mapped('partner_id')
        for partner in supp:
            partner.write({'property_stock_supplier': 263})

        supp = self.env['purchase.order'].sudo().search([('request_type', '=', 'Foregin'), ('company_id', '=', 2)]).mapped('partner_id')
        for partner in supp:
            partner.write({'property_stock_supplier': 843})

        supp = self.env['purchase.order'].sudo().search([('request_type', '=', 'Local'), ('company_id', '=', 2)]).mapped('partner_id')
        for partner in supp:
            partner.write({'property_stock_supplier': 842})
