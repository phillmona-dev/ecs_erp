...
class droga_crm_grade_vs_schedule(models.Model):
    ...
    store_reference=fields.Date('Store reference')
...
when 'pmg' then 'Project Engineer' end as state,issue_date as trans_date,
...
when 'waiting' then 'Requested' when 'sc' then 'Sent to CU' when 'reject' then 'Rejected' when 'processed' then 'Processed'
when 'pmg' then 'Project Engineer' end as state,receipt_date as trans_date,
