...
class MarketAnalysis(models.Model):
    ...
    mark_an_number = fields.Char(string='Market Analysis Number', readonly=True)
    ...
    pfi_date = fields.Date(string='Date of PFI', date_format='%d-%m-%Y')
    ip_approval_date = fields.Date(string='Date of IP Approval', date_format='%d-%m-%Y')
    date_sold = fields.Date("Date Sold", date_format='%d-%m-%Y')
    ...
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('droga.reg.market.analysis.sequence')

        vals['mark_an_number'] = sequence
        return super(MarketAnalysis, self).create(vals)
...
class AddSale(models.Model):
    ...
    product_type = fields.Selection(
        [('medicine', 'Medicine'), ('medical_device', 'Medical Device'), ('unassigned', 'Unassigned')],
