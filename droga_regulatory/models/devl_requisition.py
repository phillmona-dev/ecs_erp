...
class droga_bussiness_dev_requests(models.Model):
    ...
    def create(self, vals_list):
        vals_list['request_no'] = self.env['ir.sequence'].next_by_code(
            'droga.reg.master.custom.sequence')
        if not vals_list['request_no']:
    ...
    request_no=fields.Char('Request ID', default=lambda self: self.env['ir.sequence'].next_by_code('droga.reg.master.custom.sequence'),
                              readonly=True)
