...
class LoanDailyReport(models.Model):
    ...
    def init(self):
        ...
        self.env.cr.execute("""CREATE or REPLACE VIEW %s as (%s)""" % (self._table,
                                                                       """ select row_number()over() as id,a.id as identifier,b.acount_loan_id,d.value_date,c.value_date as date,b.value_date as date1,'' as description,c.receipt,b.value_date as payment_date,e.principal_repayment,'' as cumulative_balance,
...
from account_loan a 
inner join account_loan_repayment b on a.id=b.acount_loan_id
inner join account_loan_repayment_detail e on b.id=e.acount_loan_id
inner join account_loan_receipt c on a.id=c.acount_loan_id
inner join account_loan_int d on a.id=d.acount_loan_id
